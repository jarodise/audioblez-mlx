...
# Model IDs
KOKORO_MODEL_ID = "prince-canuma/Kokoro-82M"
...
class ChatterboxPipelineWrapper:
    ...
    def __call__(self, text, voice=None, speed=1.0, split_pattern=None):
        ...
        try:
            ...
            # Find the generated audio file (mlx_audio appends _000, _001, etc.)
...
def create_tts_pipeline(engine, voice, ref_audio=None, ref_text=None):
    """Factory function to create the appropriate TTS pipeline.

    Args:
        engine: TTS engine to use ('kokoro' or 'chatterbox')
        voice: Voice ID for Kokoro (e.g., 'af_sky')
        ref_audio: Path to reference audio for voice cloning (Chatterbox only)
        ref_text: Transcript of reference audio

    Returns:
        A pipeline object with __call__ method that yields (graphemes, phonemes, audio)
    """
    if engine == ENGINE_CHATTERBOX:
        ...
        return ChatterboxPipelineWrapper(ref_audio=ref_audio, ref_text=ref_text)
    else:
        # Default to Kokoro
        if USE_MLX:
            model = load_model(KOKORO_MODEL_ID)
            pipeline = KokoroPipeline(
                lang_code=voice[0], model=model, repo_id=KOKORO_MODEL_ID
            )
        else:
            pipeline = KPipeline(lang_code=voice[0])
        print(f"🎤 Using Kokoro engine with voice: {voice}")
        return pipeline
        ...
        if os.environ.get("ESPEAK_LIBRARY"):
            ...
            try:
                ...
                if not (library := next(iter(glob(str(pattern))), None)):
                    raise RuntimeError(
                        "No espeak-ng library found; please set the path manually"
                    )
            except (subprocess.CalledProcessError, FileNotFoundError) as e:
                raise RuntimeError(
                    "Cannot locate Homebrew Cellar. Is 'brew' installed and in PATH?"
                ) from e
...
def main(
    ...
    """Main function to generate audiobook from EPUB.

    Args:
        engine: TTS engine to use ('kokoro' or 'chatterbox')
        ref_audio: Path to reference audio for voice cloning (Chatterbox only)
        ref_text: Transcript of reference audio (auto-transcribed if not provided)
    """
    if engine is None:
    ...
    if cover_maybe:
        print(
            f"Found cover image {cover_maybe.file_name} in {cover_maybe.media_type} format"
        )
    ...
    # Initialize the TTS pipeline based on engine selection
    pipeline = create_tts_pipeline(engine, voice, ref_audio, ref_text)
    ...
    for i, chapter in enumerate(selected_chapters, start=1):
        ...
        xhtml_file_name = (
            chapter.get_name().replace(" ", "_").replace("/", "_").replace("\\", "_")
        )
        chapter_mp3_path = Path(output_folder) / filename.replace(
            extension, f"_chapter_{i}_{voice}_{xhtml_file_name}.mp3"
        )
        ...
        if audio_segments:
            ...
            failed_chapters.append(
                (i, chapter_mp3_path, ["No audio segments generated"])
            )
        ...
        tabulate(
            [
                [
                    ...
                    ok if c in chapters else "",
                    chapter_beginning_one_liner(c),
                ]
...
def gen_audio_segments(
    ...
    for i, sent in enumerate(sentences):
        ...
        for gs, ps, audio in pipeline(
            sent.text, voice=voice, speed=speed, split_pattern=r"\n\n\n"
        ):
        ...
        if stats:
            ...
            stats.eta = strfdelta(
                (stats.total_chars - stats.processed_chars) / stats.chars_per_sec
            )
            if post_event:
...
def find_document_chapters_and_extract_texts(book):
    ...
    for chapter in book.get_items():
        ...
        for text in [
            c.text.strip() for c in soup.find_all(html_content_tags) if c.text
        ]:
...
def pick_chapters(chapters):
    ...
    ret = pick(
        list(chapters_by_names.keys()), title, multiselect=True, min_selection_count=1
    )
...
def create_m4b(chapter_files, filename, cover_image, output_folder):
    ...
    proc = subprocess.run(
        [
            ...
            "-y",  # Overwrite output
            "-i",
            f"{concat_file_path}",  # Input audio
            "-i",
            f"{chapters_txt_path}",  # Input chapters
